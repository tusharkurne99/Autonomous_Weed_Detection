...
def angle_yz(x0, y0, z0, theta=None):
    ...
    # z = a + b*y
    a = (x0 * x0 + y0 * y0 + z0 * z0 + rf * rf - re * re - y1 * y1) / (2.0 *
                                                                       z0)
    b = (y1 - y0) / z0
    ...
    zj = a + b * yj
    theta = math.atan(-zj /
                      (y1 - yj)) * 180.0 / pi + (180.0 if yj > y1 else 0.0)
...
def inverse(x0, y0, z0):
    ...
    if status[0] == 0:
        theta1 = status[1]
        status = angle_yz(x0 * cos120 + y0 * sin120, y0 * cos120 - x0 * sin120,
                          z0, theta2)
    if status[0] == 0:
        theta2 = status[1]
        status = angle_yz(x0 * cos120 - y0 * sin120, y0 * cos120 + x0 * sin120,
    ...
    print [status[0], theta1, theta2, theta3]
